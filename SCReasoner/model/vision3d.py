...
class OSE3D(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.fuse.startswith('linear'):
            self.obj_proj_scene = nn.Linear(self.obj_encoder.out_dim, hidden_dim)
            # Initialize weights and biases for obj_proj_scene
            init.normal_(self.obj_proj_scene.weight, mean=0, std=0.01)
            init.normal_(self.obj_proj_scene.bias, mean=0, std=0.01)
        elif cfg.fuse.startswith('Mamba'):
            from mamba_ssm import Mamba
            self.obj_proj_scene = nn.Sequential(
                Mamba(d_model = self.obj_encoder.out_dim, d_state = 64, d_conv = 4, expand =2),
                nn.Linear(self.obj_encoder.out_dim, hidden_dim)
            )
            # Initialize weights and biases for obj_proj_scene
            init.normal_(self.obj_proj_scene[1].weight, mean=0, std=0.01)
            init.normal_(self.obj_proj_scene[1].bias, mean=0, std=0.01)
            # nn.Linear(self.obj_encoder.out_dim, hidden_dim)
        else:
            raise ValueError(f"Unknown fuse method: {cfg.fuse}")
        self.fuse = cfg.fuse.split('_')[1]
    ...
    def forward(self, data_dict):
        ...
        if 'obj_fts_scene' in data_dict.keys():
            # print("obj_fts_scene")
            obj_feats_scene = self.obj_encoder(data_dict['obj_fts_scene'])
            # print(self.obj_proj.in_features, self.obj_proj.out_features)
            if self.fuse == 'star':
                obj_feats = self.obj_proj(obj_feats) * self.obj_proj_scene(obj_feats_scene)
            elif self.fuse == 'add':
                obj_feats = self.obj_proj(obj_feats) + self.obj_proj_scene(obj_feats_scene)
        else:
            # print("no obj_fts_scene")
