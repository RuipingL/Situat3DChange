    ...
    def train_step(self, epoch):
        ...
        for data_dict in loader:
            with self.accelerator.accumulate(self.model):
                ...
                pbar.update(1)
                # del data_dict, loss, loss_all, loss_txt, loss_eai
                # gc.collect()
        ...
        for task_name in self.evaluators.keys():
            if task_name in self.data_loaders['val']:
                ...
                _, results = self.evaluators[task_name].record(
                    split='val', is_main_process=self.accelerator.is_main_process, epoch=epoch+1
                )
    ...
    def test_step(self):
        ...
        for task_name in self.evaluators.keys():
            if task_name in self.data_loaders['test']:
                ...
                _, results = self.evaluators[task_name].record(
                    split='test', is_main_process=self.accelerator.is_main_process, epoch=5
                )
            ...
            if task_name in self.data_loaders['val']:
                ...
                _, results = self.evaluators[task_name].record(
                    split='val', is_main_process=self.accelerator.is_main_process, epoch=5
                )
        ...
        if self.mode == 'train':
            ...
            for epoch in range(start_epoch, self.epochs):
                ...
                if (epoch + 1) % self.val_interval == 0:
                    ...
                    self.save(f'{epoch+1}.pth', model_only=True)   # save the latest checkpoint
                    if is_best:
