...
def rotate_translate_situ(theta, translation_vector, situ):
    situ_new = situ
    ...
    rotation_matrix = np.array([[cos_t, -sin_t, 0],
                        [sin_t, cos_t, 0],
                        [0, 0, 1]])
    situ['location'] = list(np.array(situ['location']).dot(rotation_matrix) - translation_vector)
    situ['orientation'] = list(np.array(situ['orientation']).dot(rotation_matrix))
    return situ_new
...
def direction_to_quaternion(direction):
    ...
    if norm_dir == 0:
        raise ValueError("Zero-length direction vector not allowed.")
    direction /= norm_dir
    ...
    # If the norm of the axis is zero, direction is parallel or anti-parallel to reference
    if axis_norm == 0:
        if np.allclose(direction, ref_vector):
            return np.array([0, 0, 0, 1])  # No rotation needed
        else:
            return np.array([1, 0, 0, 0])  # 180 degree rotation around any perpendicular axis
    
    axis /= axis_norm
    
    # Dot product to get the cosine of the angle of rotation
    cos_theta = np.dot(ref_vector, direction)
...
def situation_set():
    root = '/hkfs/work/workspace/scratch/zp8650-3d_change_qa/dataset/Ours/annotation/Labels/situations'
    situation_set = {}
    for filename in os.listdir(root):
        ...
        if filename.endswith('.json'):
            ...
            for scene_id in data.keys():
                for response in data[scene_id]['response']:
                    if response['scan_id'] not in situation_set.keys():
                        situation_set[response['scan_id']] = {}
                    if response['situation'].startswith('standing'):
                        pattern = r"^(standing with )(.+)( \d+ o'clock)$"
                        match = re.match(pattern, response['situation'])
                        if match:
                            prefix, middle, suffix = match.groups()
                            # Split the middle part by whitespace and join with underscores
                            new_middle = '_'.join(middle.split())
                            response['situation'] = prefix + new_middle + suffix  
                    elif response['situation'].startswith('sitting'):
                        prefix = 'sitting on '
                        # elif response['situation'].startswith('interacting with'):
                        #     prefix = 'interacting with '
                        # print(response['scan_id'], response['situation'])
                        remainder = response['situation'][len(prefix):]
                        new_remainder = '_'.join(remainder.split())
                        response['situation'] = prefix + new_remainder
                        # print(response['situation'])
                    situation_set[response['scan_id']][response['situation']] = {"location": response['location'], "orientation": response['orientation']}
